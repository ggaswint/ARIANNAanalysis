...
import voltageToEfieldAnalyticConverterForNeutrinos
...
det = detector.Detector(json_filename = PathToARIANNAanalysis + "/configurations/ARIANNA_4LPDA_1dipole.json", antenna_by_depth=False)
...
def forward_folding(event):
    dataNurFile = PathToARIANNAanalysis + "/data/triggeredNeutrinoEvents.nur"
    dataHDF5File = PathToARIANNAanalysis + "/data/triggeredNeutrinoEvents.hdf5"
    savFile = PathToARIANNAanalysis + "/data/reconstructedNeutrinoProperties"
    ...
    for station in evt.get_stations():
        ...
        shower_energy_reco = station.get_parameter(stnp.nu_energy) # Note this is the shower energy, although stored as nu energy needs to be fixed
