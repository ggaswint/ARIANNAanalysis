...
class voltageToAnalyticEfieldConverterNeutrinos:
    ...
    def run(self, evt, station, det, icemodel, shower_type='HAD', use_channels=[0,1,2,3], attenuation_model='SP1',
        ...
        use_channels: array of ints
            the channel ids for the antennas used for electric field reconstruction
        attenuation_model:
            the attenuation model used
        parametrization:
            parametrization model used for the reconstruction. At the moment only optimized for Alvarez2009
        hilbert: bool
            use the hilbert envelope of the traces in the chi2 fit
        use_bandpass_filter: bool
            if True, the voltage traces are filtered according to the passband_low and passband_high inputs
        passband_low: map between channel id and complex filter amplitudes which themselves are array of complex floats
            if set, the length should correspond to the length of use_channels, the filters are used in order of a SORTED use_channels order
        passband_low: map between channel id and complex filter amplitudes which themselves are array of complex floats
            if set, the length should correspond to the length of use_channels, the filters are used in order of a SORTED use_channels order
        include_focusing: bool
            include amplification of signal due to focusing effects.
        use_MC: bool
        ...
        n_samples_multiplication_factor: int
            increasing samples is necesseary to prevent rolling issues with reflected signals that are way beyond the current time window for certain sites like Moore's bay
            and detector setups where the cable delay and antenna position differences between channels is large. This is a consequence of considering all ray tracing
            solutions for each reconstruction and correlating the best matching one to the true signal. Without expanding n_samples, reflected solutions that are say
            microseconds apart can wrap just right to interfere with a direct solution and cause poor results in reconstruction.
        plot_traces_with_true_input: bool
            if True produces a figure overlaying the simulated voltage traces with the reconstructed voltage traces using the true nu direction and energy
        debug: bool
            if True debug plotting is enables
        """

        def minimizer(params, n_index, hilbert = False, minimizer=True):
            """
            ...
            """
            thetas = np.zeros((n_antennas, maxNumRayTracingSolPerChan))
        ...
        def plotSimulatedVersusReconstructedTraces(nu_zenith, nu_azimuth, shower_energy, save):
            traces, viewing_angles_reco = minimizer([nu_zenith, nu_azimuth, np.log10(shower_energy)], n_index,hilbert=hilbert,minimizer = False)
        ...
        # Expand trace window by this factor. Necessary due to global time offset between simulated and reconstructed traces.
        # For example: If reco trace has reflected signal just outside of non expanded window, then it would not be kept in the reco trace,
        #     BUT if after adding global time offset, it should appear to be within the window, then now we have lost this information.
        #     We need to increase our window size by at least the amount of the maximum global time offset that could be applied
        #     2 should suffice (Note the origional trace is centered on this expanded window)
        # Note that the length of ff is dependent on this, if using a band pass filter insure ff is the same length
        # Note 2; for Moore's Bay, reflected signals can be microseconds apart. Should set n_expand to ~ 50 to prevent rolling. 50 is from largest delta time between pulses divided by (2 * trace time window). (2 because centered).
        ...
        if use_MC and (station.get_sim_station() is not None):
            ...
            # Note they are not ordered in any particular way so this is first gathered and ordered by storing in a map with channel id as key
            ...
            # Sometimes some events do not have ray tracing solutions to all channels in use channels.
            # This block of code catches that so the following code does not try to access data for channels which does not exist
            use_channels_tmp = []
        ...
        for iA, position in enumerate(antenna_positions):
            r = ray.ray_tracing(vertex_position, position, icemodel,attenuation_model=attenuation_model, n_frequencies_integration=25,n_reflections=n_reflections)
            r.find_solutions()
        ...
        station.set_parameter(stnp.nu_zenith,results[0][0])
        station.set_parameter(stnp.nu_azimuth,results[0][1])
        #Note that this is actually the shower energy. A new parameter will need to be made for this.
        station.set_parameter(stnp.nu_energy,10.0**results[0][2])
        ...
        if debug:
            ...
            traces, viewing_angles_reco = minimizer([nu_zenith, nu_azimuth, 10.0**shower_energy], n_index, hilbert=hilbert, minimizer=False)
            ...
            plotSimulatedVersusReconstructedTraces(nu_zenith, nu_azimuth, shower_energy, save)
            ...
            for iC, c in enumerate(zenith):
                for iR, r in enumerate(azimuth):
                    zmesh[iR][iC] = np.log(minimizer([c, r, shower_energy], n_index,hilbert=hilbert))
            ...
            for iC, c in enumerate(energy):
                for iR, r in enumerate(zenith):
                    zmesh[iR][iC] = np.log(minimizer([r, nu_azimuth, c], n_index,hilbert=hilbert))
            ...
            for iC, c in enumerate(energy):
                for iR, r in enumerate(azimuth):
                    zmesh[iR][iC] = np.log(minimizer([nu_zenith, r, c], n_index,hilbert=hilbert))
